...
export async function importOrderSheet(env,{clientId,storeId,source,rows,actor}){
  ...
  for(let index=0;index<rows.length;index++){
    ...
    try{
      const ref=importRef(source,r,index),old=await existingOrder(env,{clientId,storeId,source,row:r,ref}),state=stateOf(r.state||r.status),qty=Math.max(1,Number(r.qty)||1),match=catalogMatch(products,r),productCost=match?r2((Number(match.cost)||0)*qty):0,customerId=await findOrCreateCustomer(env,{clientId,storeId,name,rawPhone,gov:text(r.gov),address:text(r.address)}),history=historyOf(old?.history),coupon=OPTIONAL_PLACEHOLDER(r.coupon),couponDiscount=coupon?(num(r.couponDiscount)||0):0;
