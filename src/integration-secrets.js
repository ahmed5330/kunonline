...
async function keyFromEnv(env){
  ...
  if(!raw){
    if(env.APP_ENV==='preview'&&env.SESSION_SECRET){
      const bytes=await crypto.subtle.digest('SHA-256',enc.encode(`kunonline-preview-integration-v1:${env.SESSION_SECRET}`));
      return crypto.subtle.importKey('raw',bytes,{name:'AES-GCM'},false,['encrypt','decrypt']);
    }
    throw Object.assign(new Error('INTEGRATION_ENCRYPTION_KEY غير مهيأ'),{status:503,code:'INTEGRATION_KEY_MISSING'});
  }
