...
async function clearMetaCache(env,clientId){
  await env.DB.batch([
    env.DB.prepare("DELETE FROM campaign_daily_metrics WHERE client_id=? AND campaign_id IN (SELECT id FROM marketing_campaigns WHERE client_id=? AND platform='meta_ads')").bind(clientId,clientId),
    env.DB.prepare("DELETE FROM marketing_campaigns WHERE client_id=? AND platform='meta_ads'").bind(clientId)
  ]);
}
async function persistValidation(env,clientId,connection,result,currentConfig){
  const ts=now(),status=result.status||'configured',connected=status==='connected',isMeta=connection.provider==='meta_ads';
  const nextConfig={...(currentConfig||{}),...(result.config||{})};
  if(isMeta&&!connected){delete nextConfig.adAccountId;delete nextConfig.ad_account_id;nextConfig.adAccountConfirmed=false;await env.DB.prepare('UPDATE store_connections SET status=?,external_store_id=NULL,config_json=?,last_sync_at=NULL,last_error=?,updated_at=? WHERE id=? AND client_id=?').bind(status,JSON.stringify(nextConfig),result.ok?null:(result.message||'Validation failed'),ts,connection.id,clientId).run();return;}
  if(isMeta&&connected){
    const previous=cleanAccountId(currentConfig?.adAccountId||currentConfig?.ad_account_id),selected=cleanAccountId(result?.config?.adAccountId);
    if(currentConfig?.adAccountConfirmed!==true||previous!==selected)await clearMetaCache(env,clientId);
  }
  await env.DB.prepare(`UPDATE store_connections SET status=?,store_name=COALESCE(?,store_name),external_store_id=COALESCE(?,external_store_id),config_json=?,last_sync_at=?,last_error=?,updated_at=? WHERE id=? AND client_id=?`).bind(status,connected?(result.storeName||null):null,connected?(result.externalStoreId||null):null,JSON.stringify(nextConfig),connected?ts:(connection.last_sync_at||null),result.ok?null:(result.message||'Validation failed'),ts,connection.id,clientId).run();
}
async function fetchV19(request,env,ctx){
  ...
  try{
    ...
    const selectedAdAccountId=b.adAccountId||b.ad_account_id||null;
    const result=await validateProviderConnection({env,provider,secrets,selectedAdAccountId});
    await persistValidation(env,clientId,connection,result,currentConfig);
    await audit(env,m,clientId,connectionId,{provider:provider.id,ok:!!result.ok,status:result.status,externalConnectivityChecked:!!result.externalConnectivityChecked,requiresAccountSelection:!!result.requiresAccountSelection,requiresAccountIdInput:!!result.requiresAccountIdInput,adAccountId:result.account?.accountId||null,code:result.code||null});
