...
async function workflows(request,env,me,url){
  ...
  if(request.method==='GET'){const {results}=await env.DB.prepare('SELECT id,name,trigger_type,definition_json,active,created_by,created_at,updated_at FROM workflows WHERE client_id=? ORDER BY created_at DESC').bind(clientId).all();return json((results||[]).map(x=>({...x,definition:JSON.parse(x.definition_json||'{}')})));}
  if(!canWriteOps(me))return json({error:'مش مسموح'},403);
  const b=await request.json().catch(()=>({}));if(!b.name||!b.triggerType)return json({error:'الاسم والـTrigger مطلوبان'},400);const wid=id('WF'),ts=now();
  const definition={conditions:Array.isArray(b.conditions)?b.conditions:[],actions:Array.isArray(b.actions)?b.actions:[]};await env.DB.prepare(`INSERT INTO workflows (id,client_id,name,trigger_type,definition_json,active,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)`).bind(wid,clientId,String(b.name).trim(),String(b.triggerType),JSON.stringify(definition),b.active?1:0,me.email||me.uid||'',ts,ts).run();await audit(env,me,clientId,'workflow.create','workflow',wid,null,{name:b.name,triggerType:b.triggerType});return json({ok:true,id:wid},201);
}
