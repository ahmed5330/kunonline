...
export async function collectedProfitOverview(env,{clientId,storeId=null,from=null,to=null,includeDetails=false}={}){
  ...
  const orders=ordersResult.results||[],transactions=txResult.results||[];
  ...
  const manualAdTransactions=transactions.filter(row=>isAdCategory(row.category)),manualAds=r2(manualAdTransactions.reduce((sum,row)=>sum+num(row.amount),0)),integratedAds=r2(adsRow?.spend),adSpend=integratedAds>0?integratedAds:manualAds,adSpendSource=integratedAds>0?'integrations':'manual';
