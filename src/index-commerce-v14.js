...
async function supplierBalances(env,clientId){
  const {results=[]}=await env.DB.prepare(`
    ...
    FROM suppliers s
    LEFT JOIN (SELECT supplier_id,SUM(total) invoiced FROM supplier_invoices WHERE client_id=? GROUP BY supplier_id) i ON i.supplier_id=s.id
    LEFT JOIN (SELECT supplier_id,SUM(amount) paid FROM supplier_payments WHERE client_id=? GROUP BY supplier_id) p ON p.supplier_id=s.id
    LEFT JOIN (SELECT supplier_id,SUM(total) returned FROM purchase_returns WHERE client_id=? AND status='completed' GROUP BY supplier_id) r ON r.supplier_id=s.id
...
async function fetchV14(request,env,ctx){
  ...
  try{
    if(path==='/api/procurement/supplier-balances'&&method==='GET'){
      ...
      return json(await supplierBalances(env,clientId));
    }
