...
async function releaseContact({request,env,ctx,delegate,orderId,url}){
  ...
  const result=await env.DB.prepare('UPDATE orders SET contact_claim_user_id=NULL,contact_claim_name=NULL,contact_claimed_at=NULL WHERE id=? AND client_id=? AND contact_claim_user_id=?').bind(orderId,clientId,mine).run();
  const released=Number(result?.meta?.changes||0)>0,at=new Date().toISOString();
  if(released){try{await env.DB.prepare('INSERT INTO audit_log (id,client_id,store_id,actor_user_id,actor_email,action,entity_type,entity_id,metadata_json,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)').bind(rid('AUD'),clientId,row.store_id||null,me?.uid||me?.id||null,me?.email||me?.role||null,'order.contact_release','order',orderId,JSON.stringify({returnState:row.state||'pending'}),at).run();}catch{}}
  return json({ok:true,orderId,state:row.state,released,returnedToOriginalState:true});
}
