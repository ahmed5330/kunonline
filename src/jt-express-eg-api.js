...
function senderFrom(fields){const mobile=normalizePhone(fields.senderMobile),phone=normalizePhone(fields.senderPhone)||mobile;return compact({name:fields.senderName,company:fields.senderCompany||fields.senderName,mobile,phone,countryCode:'EGY',prov:fields.senderProv,city:fields.senderCity,area:fields.senderArea,street:fields.senderStreet});}

export function buildJtCreatePayload(shipment={},secrets={}){
  const {fields,missing,enterpriseReady,senderReady}=jtCredentials(secrets);if(missing.length)throw Object.assign(new Error('J&T Express يحتاج API Account وPrivate Key وSource Code'),{status:409,code:'JT_CREDENTIALS_MISSING'});
  ...
  if(senderReady)payload.sender=senderFrom(fields);
  return payload;
}
async function readResponse(response){const text=await response.text().catch(()=>''),data=(()=>{try{return text?JSON.parse(text):{};}catch{return {raw:text};}})(),raw=clean(data?.raw||text,700);return {response,data,code:clean(data?.code??data?.resultCode??data?.statusCode,80),message:clean(data?.msg||data?.message||data?.error||raw,700)};}
async function signedPost(path,payload,secrets,{fetcher=fetch}={}){const {fields,missing}=jtCredentials(secrets);if(missing.length)throw Object.assign(new Error('بيانات J&T الأساسية غير مكتملة'),{status:409,code:'JT_CREDENTIALS_MISSING'});const bizContent=JSON.stringify(payload),timestamp=String(Math.floor(Date.now()/1000));let response;try{response=await fetcher(`${LIVE_BASE}${path}`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded',apiAccount:fields.apiAccount,digest:md5Base64(bizContent+fields.privateKey),timestamp},body:new URLSearchParams({bizContent}).toString()});}catch(error){throw Object.assign(new Error(`تعذر الاتصال بـ J&T: ${clean(error?.message,300)||'network error'}`),{status:502,code:'JT_NETWORK_ERROR',cause:error});}return {...await readResponse(response),path,bizContent};}
...
export async function createJtShipment({shipment,secrets,fetcher=fetch}){const payload=buildJtCreatePayload(shipment,secrets),txlogisticId=payload.txlogisticId;let result;try{result=await signedPost(ADD_ORDER_PATH,payload,secrets,{fetcher});}catch(error){const recovered=await recoverExisting(txlogisticId,secrets,{fetcher});if(recovered)return {ok:true,...recovered,txlogisticId:recovered.txlogisticId||txlogisticId,payload};throw error;}if(result.response.status===404||result.response.status===405)result=await signedPost(CREATE_ORDER_PATH,payload,secrets,{fetcher});if(success(result)){let parsed=parseJtShipmentResult(result.data);if(!parsed.awb){const recovered=await recoverExisting(txlogisticId,secrets,{fetcher});if(recovered)return {ok:true,...recovered,txlogisticId:recovered.txlogisticId||txlogisticId,payload,path:result.path};throw Object.assign(new Error('J&T قبلت الطلب لكن لم ترجع Bill Code (رقم البوليصة) في الاستجابة، ولم يظهر الرقم بعد في Get Orders. لم ننقل الأوردر إلى جاري الشحن حتى لا نفقد رقم التتبع.'),{status:502,code:'JT_AWB_MISSING',jtResponse:result.data});}return {ok:true,...parsed,txlogisticId:parsed.txlogisticId||txlogisticId,payload,raw:result.data,path:result.path};}if(duplicateLike(result)){const recovered=await recoverExisting(txlogisticId,secrets,{fetcher});if(recovered)return {ok:true,...recovered,txlogisticId:recovered.txlogisticId||txlogisticId,payload};}const needsEnterprise=enterpriseCredentialHint(result),hint=needsEnterprise?' حساب J&T نفسه طلب Customer Code/Customer Password؛ وقتها فقط نحتاجهم من J&T Enterprise Info.':'';throw Object.assign(new Error(`J&T رفضت إنشاء الشحنة${result.code?` (${result.code})`:''}: ${result.message||`HTTP ${result.response.status}`}.${hint}`),{status:result.response.status>=500?502:422,code:'JT_CREATE_REJECTED',jtCode:result.code,httpStatus:result.response.status,jtResponse:result.data,enterpriseCredentialsRequired:needsEnterprise});}
export async function trackJtShipment({awb,secrets,fetcher=fetch}){const code=clean(awb,160);if(!code)throw Object.assign(new Error('رقم البوليصة / Bill Code مطلوب للتتبع'),{status:400,code:'JT_AWB_REQUIRED'});const {fields,enterpriseReady}=jtCredentials(secrets),payload=enterpriseReady?{customerCode:fields.customerCode,digest:businessDigest(fields.customerCode,fields.customerPassword,fields.privateKey),billCodes:code,sourceCode:fields.sourceCode}:{sourceCode:fields.sourceCode,billCodes:code};const result=await signedPost(TRACE_PATH,payload,secrets,{fetcher});if(!success(result))throw Object.assign(new Error(`تعذر جلب تتبع J&T${result.code?` (${result.code})`:''}: ${result.message||`HTTP ${result.response.status}`}`),{status:result.response.status>=500?502:422,code:'JT_TRACK_REJECTED',jtCode:result.code,jtResponse:result.data});return {ok:true,awb:code,data:result.data};}
export const __jtApiInternals={ADD_ORDER_PATH,CREATE_ORDER_PATH,GET_ORDERS_PATH,TRACE_PATH,LIVE_BASE,businessDigest,signedPost,success,parseJtShipmentResult,senderFrom};
