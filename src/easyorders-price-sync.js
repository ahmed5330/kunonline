...
const nullable=value=>value===null||value===undefined||value===''?null:num(value);
...
async function syncConnection(env,row){
  ...
  for(const product of pulled.products){
    const importedId=await stableId('IMP',row.client_id,storeId,'easyorders',product.externalId||product.sku),match=await env.DB.prepare("SELECT id,price,compare_at_price FROM products WHERE client_id=? AND store_id IS ? AND (id=? OR (?<>'' AND LOWER(sku)=LOWER(?))) LIMIT 1").bind(row.client_id,storeId,importedId,product.sku||'',product.sku||'').first();
    if(!match)continue;const nextPrice=num(product.price),nextCompare=nullable(product.compareAtPrice),currentCompare=nullable(match.compare_at_price);if(Number(match.price)!==nextPrice||currentCompare!==nextCompare){await env.DB.prepare('UPDATE products SET price=?,compare_at_price=?,updated_at=? WHERE id=? AND client_id=?').bind(nextPrice,nextCompare,new Date().toISOString(),match.id,row.client_id).run();result.productsChanged++;}
    for(const [index,variant] of (product.variants||[]).entries()){
      const importedVariantId=await stableId('IMV',row.client_id,storeId,'easyorders',product.externalId||product.sku,variant.externalId||variant.sku||index),variantMatch=await env.DB.prepare("SELECT id,price,compare_at_price FROM product_variants WHERE client_id=? AND store_id IS ? AND product_id=? AND (id=? OR (?<>'' AND LOWER(sku)=LOWER(?))) LIMIT 1").bind(row.client_id,storeId,match.id,importedVariantId,variant.sku||'',variant.sku||'').first();
      if(!variantMatch)continue;const nextVariantPrice=num(variant.price),nextVariantCompare=nullable(variant.compareAtPrice),current=variantMatch.price===null?null:Number(variantMatch.price),currentVariantCompare=nullable(variantMatch.compare_at_price);if(current!==nextVariantPrice||currentVariantCompare!==nextVariantCompare){await env.DB.prepare('UPDATE product_variants SET price=?,compare_at_price=?,updated_at=? WHERE id=? AND client_id=?').bind(nextVariantPrice,nextVariantCompare,new Date().toISOString(),variantMatch.id,row.client_id).run();result.variantsChanged++;}
    }
  }
  return result;
}
export async function syncEasyOrdersPrices(env,{clientId=null,limitConnections=40}={}){
  ...
  const summary={ok:true,connections:results.length,synced:0,skipped:0,productsChanged:0,variantsChanged:0,pricePolicy:'sale_price_when_positive_else_price',errors:[]};
