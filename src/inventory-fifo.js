...
async function refreshBatchStatus(env,batchId){
  ...
  await env.DB.prepare('UPDATE inventory_batches SET status=? WHERE id=?').bind(num(row?.remaining)>0?'active':'depleted',batchId).run();
}
...
async function resolveLegacyOrderProduct(env,order){
  ...
  if(results.length!==1)fail('تعذر تحديد منتج الأوردر داخل المخزون بدقة. اختاره من شاشة تأكيد الطلب أولًا.',409,'ORDER_PRODUCT_MATCH_AMBIGUOUS');
  await env.DB.prepare('UPDATE orders SET product_id=? WHERE id=? AND client_id=?').bind(results[0].id,order.id,order.client_id).run();
  return {productId:results[0].id,variantId:null};
}
async function resolveLineProduct(env,order,line){
  if(line.product_id)return line;const sku=clean(line.sku),name=clean(line.product_name);
  if(sku){
  ...
  fail(`المنتج «${name||sku||'غير معروف'}» غير مربوط بمنتج/متغير في المخزون. اختاره من شاشة التأكيد أولًا.`,409,'ORDER_ITEM_PRODUCT_NOT_LINKED');
}
...
async function stockLog(env,{clientId,storeId,productId,variantId,productName,delta,newStock,lot,orderId,actor,note}){
  const id=rid('STK'),at=stamp();
  await env.DB.prepare('INSERT INTO stock_log (id,client_id,store_id,product_id,variant_id,product_name,delta,new_stock,note,supplier_id,supplier_name,stock_date,batch_id,batch_name,created_at,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,clientId,storeId,productId,variantId||null,productName||'',delta,newStock,note,null,null,at.slice(0,10),lot?.batch_id||lot?.batchId||null,lot?.batch_name||lot?.batchName||null,at,actorName(actor)).run();return id;
}
...
async function rollbackSlices(env,token){
  for(const slice of [...(token?.slices||[])].reverse()){
    ...
    if(slice.generalAdjusted){
      if(slice.variantId)await env.DB.prepare('UPDATE product_variants SET stock=COALESCE(stock,0)+? WHERE id=? AND client_id=?').bind(slice.qty,slice.variantId,token.clientId).run().catch(()=>{});
      else await env.DB.prepare('UPDATE products SET stock=COALESCE(stock,0)+? WHERE id=? AND client_id=?').bind(slice.qty,slice.productId,token.clientId).run().catch(()=>{});
      if(slice.logId)await env.DB.prepare('DELETE FROM stock_log WHERE id=?').bind(slice.logId).run().catch(()=>{});
    }
...
export async function prepareOrderStockTransition(env,{clientId,storeId,orderId,fromState,toState,actor}={}){
  ...
  const token={kind:'allocated',fifo:true,orderId,clientId,storeId,qty:0,slices:[],batchIds:[],batchNames:[],previousLegacy:legacy||null,legacySummaryWritten:false};
  try{
    for(const group of groups.values()){
      const lots=group.lots.map(lot=>({...lot,virtualRemaining:num(lot.remaining_qty)}));
      for(const line of group.lines){let left=line.qty;for(const lot of lots){if(left<=0)break;if(lot.virtualRemaining<=0)continue;const take=Math.min(left,lot.virtualRemaining),changed=await env.DB.prepare('UPDATE inventory_batch_items SET remaining_qty=remaining_qty-? WHERE id=? AND remaining_qty>=?').bind(take,lot.id,take).run();if(Number(changed?.meta?.changes||0)!==1)fail('المخزون اتغير أثناء تأكيد الطلب. أعد المراجعة والمحاولة.',409,'STOCK_FIFO_CONCURRENT_CHANGE');
        const newStock=await changeStock(env,{clientId,productId:group.productId,variantId:group.variantId},-take),logId=await stockLog(env,{clientId,storeId,productId:group.productId,variantId:group.variantId,productName:lot.product_name||line.product_name,delta:-take,newStock,lot,orderId,actor,note:`خصم أوردر ${orderId} تلقائيًا بنظام FIFO عند تأكيد الطلب — ${lot.batch_name}`});
        ...
        token.slices.push({allocationId,batchId:lot.batch_id,batchItemId:lot.id,batchName:lot.batch_name,productId:group.productId,variantId:group.variantId,qty:take,generalAdjusted:true,logId});token.qty+=take;if(!token.batchIds.includes(lot.batch_id)){token.batchIds.push(lot.batch_id);token.batchNames.push(lot.batch_name);}lot.virtualRemaining-=take;left-=take;await refreshBatchStatus(env,lot.batch_id);}
        if(left>0)fail('تعذر إكمال خصم الطلب من دفعات المخزون',409,'STOCK_FIFO_ALLOCATION_INCOMPLETE');
      }
    }
...
async function restoreGeneralForAllocation(env,allocation,{clientId,storeId,orderId,actor}){
  const productId=allocation.product_id,variantId=allocation.variant_id||null,qty=num(allocation.qty);if(qty<=0||!productId)return;
  const newStock=await changeStock(env,{clientId,productId,variantId},qty);
...
async function releaseLegacy(env,{clientId,orderId,toState,actor,restoreGeneral}){
  ...
  return {kind:status,batchId:legacy.batch_id,qty:amount};
}
