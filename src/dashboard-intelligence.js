...
export function computeDashboardSnapshot({orders=[],historyOrders=[],transactions=[],billingRows=[],products=[],dailyAds=[],marketing={total:{},campaigns:[]},aiSnapshots=[],from,to,today=cairoToday(),currency='EGP'}){
  ...
  const latestAi=[];
  for(const row of aiSnapshots||[]){const payload=safeJson(row.suggested_payload_json,{}),embedded=Array.isArray(payload.recommendations)?payload.recommendations:[];latestAi.push({id:row.id,category:row.insight_type||'ai',severity:row.severity||'info',title:row.title,rationale:row.rationale||'',generatedAt:row.generated_at});for(const rec of embedded.slice(0,3))latestAi.push({id:`${row.id}:${latestAi.length}`,category:rec.type||'ai',severity:rec.severity||'info',title:rec.title||row.title,rationale:rec.detail||'',action:rec.action||null,generatedAt:row.generated_at});}
  const recommendations=[...latestAi.slice(0,6),...derivedRecommendations({overview,ads,rates,provinces})].slice(0,10);
  ...
  return {
    ...
    overview:{...overview,details:{
      ...
      adSpend:[{label:'صرف الإعلانات',value:adSpend,money:true},{label:'مصدر الرقم',text:adSpendSource==='integrations'?'بيانات منصات الإعلانات المتصلة':'المصاريف الإعلانية المسجلة يدويًا'},{label:'الحملات',items:ads.campaigns.slice(0,8).map(c=>({label:c.name||c.platform,value:r2(c.spend)}))}],
    ...
    ads,rates,provinces,recommendations,trend
  };
}
