...
export async function persistBrief(env,{clientId,storeId=null,brief,actor=null}){
  ...
  const title=String(brief?.summary||top.title||'Kun AI Business Brief');
  await env.DB.prepare(`INSERT INTO ai_insight_snapshots
    ...
    .bind(id,clientId,storeId||null,'business_brief',severity,title,
      String(top.detail||`Generated by ${engine}`),JSON.stringify({engine,metrics:brief?.metrics||{}}),
      top.action?'review_business_brief':null,JSON.stringify({engine,recommendations}),
      'active',ts).run();
  return id;
}
