...
export async function startPostShippingCollectionV47(env,{clientId,orderId,me}){
  ...
  return {ok:true,id:orderId,state:'signed',postShippingStage:'collecting',history};
}

export async function collectPostShippingOrderV47(env,{clientId,orderId,amount,me}){
  const row=await orderForWrite(env,{clientId,orderId,me});
  if(row.state!=='signed'||clean(row.checkpoint)!=='جاري التحصيل')fail('التحصيل متاح للطلبات الموجودة في «جاري التحصيل» فقط',409,'POST_SHIPPING_COLLECTION_STATE_INVALID');
  ...
  const at=stamp(),history=parseArray(row.history),entry={type:'cod_collection',state:'collected',stage:'collected',amount:Number(value.toFixed(2)),note:`تم استلام ${Number(value.toFixed(2))} جنيه من شركة الشحن`,at,...actor(me)};history.push(entry);
  await env.DB.prepare("UPDATE orders SET state='collected',checkpoint=?,collected_amount=?,collected_at=?,history=? WHERE id=? AND client_id=?").bind('تم التحصيل',entry.amount,at.slice(0,10),JSON.stringify(history),orderId,clientId).run();
  await audit(env,{row,me,action:'order.cod_collection',before:{state:row.state,checkpoint:row.checkpoint,collectedAmount:row.collected_amount},after:{state:'collected',checkpoint:'تم التحصيل',collectedAmount:entry.amount},metadata:{source:'post_shipping',manual:true}});
  return {ok:true,id:orderId,state:'collected',postShippingStage:'collected',collectedAmount:entry.amount,collectedAt:at,history};
}
