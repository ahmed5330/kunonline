...
object AppUpdateManager {
    ...
    private const val PREFS = "kun_mobile_update"
    ...
    fun check(activity: Activity, userInitiated: Boolean = false) {
        ...
        Thread {
            ...
            activity.runOnUiThread {
                ...
                if (info == null) {
                    if (userInitiated) Toast.makeText(activity, "تعذر التحقق من تحديث Kun Online", Toast.LENGTH_SHORT).show()
    ...
    private fun fetchInfo(): UpdateInfo {
        val connection = (URL(BASE_URL + ENDPOINT).openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("X-Kun-Mobile", "stable-android/${BuildConfig.VERSION_NAME}")
        }
        ...
        if (json.has("available") && !json.optBoolean("available", true)) {
            return UpdateInfo(BuildConfig.VERSION_CODE, BuildConfig.VERSION_NAME, "", false, emptyList())
        }
        val notesJson = json.optJSONArray("notes")
        val notes = buildList {
            if (notesJson != null) for (i in 0 until notesJson.length()) {
                notesJson.optString(i).takeIf { it.isNotBlank() }?.let(::add)
            }
        }
        val info = UpdateInfo(
        ...
        if (info.versionCode > BuildConfig.VERSION_CODE && info.apkUrl.isBlank()) error("Update URL missing")
        return info
    }

    private fun showUpdateDialog(activity: Activity, info: UpdateInfo) {
        val message = buildString {
            append("إصدار Kun Online ${info.versionName} متاح الآن.")
            if (info.notes.isNotEmpty()) {
                append("\n\n")
                info.notes.forEach { append("• ").append(it).append('\n') }
            }
            append("\nسيتم تنزيل التحديث من نظام كن أونلاين، ثم سيطلب Android موافقتك على التثبيت.")
        }.trim()
    ...
    private fun download(activity: Activity, info: UpdateInfo) {
        ...
        val dialog = AlertDialog.Builder(activity)
            ...
            .setMessage("بعد اكتمال التنزيل ستظهر شاشة تثبيت Android تلقائيًا.")
            .setCancelable(false)
            .show()
        Thread {
            val result = runCatching {
                val file = File(activity.cacheDir, "kun-online-${info.versionCode}.apk")
                downloadFile(info.apkUrl, file)
                file
            }
            activity.runOnUiThread {
                downloading = false
    ...
    private fun downloadFile(rawUrl: String, target: File) {
        ...
        repeat(8) {
            val connection = (URL(current).openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("User-Agent", "KunOnline/${BuildConfig.VERSION_NAME}")
        ...
        error("تعذر الوصول إلى ملف التحديث")
    }

    private fun requestInstall(activity: Activity, apk: File) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && !activity.packageManager.canRequestPackageInstalls()) {
            activity.getSharedPreferences(PREFS, Activity.MODE_PRIVATE).edit().putString(PENDING_APK, apk.absolutePath).apply()
            Toast.makeText(activity, "اسمح لـ Kun Online بتثبيت التطبيقات ثم ارجع للتطبيق", Toast.LENGTH_LONG).show()
