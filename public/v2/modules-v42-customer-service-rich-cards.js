/* Kun Online v42 — rich Customer Service cards matching order details */
(function(){
  const cache=new Map();
  ...
  function delivery(c={}){
    ...
    return `<div class="cs-rich-delivery ${cls}"><div class="cs-rich-delivery-head"><span>نسبة التسليم للعميل</span><span class="cs-rich-level">${level} · ${rate}%</span>${copyBtn(`${rate}%`,'نسخ نسبة التسليم')}</div><div class="cs-rich-bars">${Array.from({length:5},(_,i)=>`<span class="cs-rich-bar ${i<bars?'on':''}"></span>`).join('')}</div></div>`;
  }
  function row(value,{name=false,label='نسخ'}={}){
    if(!present(value))return '';
    return `<div class="cs-rich-row"><div class="cs-rich-value ${name?'cs-rich-name':''}">${esc(value)}</div>${copyBtn(value,label)}</div>`;
  }
  ...
  function product(item={}){
    ...
    return `<div class="cs-rich-product">${image}<div class="cs-rich-product-main"><div class="cs-rich-product-line"><div class="cs-rich-product-name">${esc(item.name||'منتج')}</div>${copyBtn(item.name||'منتج','نسخ اسم المنتج')}</div>${choice?`<div class="cs-rich-product-line"><div class="cs-rich-option">${esc(choice)}</div>${copyBtn(choice,'نسخ اللون/المقاس/الاختيارات')}</div>`:''}<div class="cs-rich-product-price"><span class="cs-rich-qty">${qty}x ${money(price)} ${copyBtn(`${qty}x ${money(price)}`,'نسخ الكمية والسعر')}</span><span class="cs-rich-line-total">${money(total)} ${copyBtn(money(total),'نسخ إجمالي المنتج')}</span></div></div></div>`;
  }
  function renderDetails(d={}){
    ...
    return `<div class="cs-rich-card"><div class="cs-rich-top">${o.storeName?`<span class="cs-rich-store">${esc(o.storeName)}</span>`:''}${row(c.name,{name:true,label:'نسخ اسم العميل'})}${row(c.phone,{label:'نسخ رقم الهاتف'})}${row(c.email,{label:'نسخ البريد الإلكتروني'})}${row(address,{label:'نسخ العنوان بالكامل'})}${delivery(c)}</div><div class="cs-rich-divider"></div><div class="cs-rich-products">${items.length?items.map(product).join(''):'<div class="cs-rich-loading">لا توجد تفاصيل منتجات إضافية.</div>'}</div><div class="cs-rich-meta"><span class="cs-rich-meta-item">${total}${copyBtn(total,'نسخ الإجمالي')}</span><span class="cs-rich-meta-item">${qty}${copyBtn(qty,'نسخ الكمية')}</span>${date?`<span class="cs-rich-meta-item">${esc(date)}${copyBtn(date,'نسخ التاريخ')}</span>`:''}</div></div>`;
  }
  ...
  window.KunCustomerServiceRichCards={scan,cache,version:'42'};
})();
