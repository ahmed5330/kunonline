...
(function(){
  ...
  async function renderApprovals(root){
    ...
    const load=async()=>{const box=root.querySelector('#apBody');try{const st=root.querySelector('#apStatus').value;const rows=await api(`/api/approvals${st?`?status=${encodeURIComponent(st)}`:''}`);if(!rows.length){box.innerHTML='لا توجد طلبات موافقة.';return;}box.className='table-wrap';box.innerHTML=`<table class="table compact"><thead><tr><th>الإجراء</th><th>المصدر</th><th>المخاطرة</th><th>الحالة</th><th>الطالب</th><th>الوقت</th><th>قرار</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${esc(x.action_type)}</td><td>${esc(x.source)}</td><td>${esc(x.risk)}</td><td>${statusBadge(x.status)}</td><td>${esc(x.requested_by||'—')}</td><td>${fmt(x.requested_at)}</td><td>${x.status==='pending'?`<button class="btn primary apDecision" data-id="${esc(x.id)}" data-d="approve">موافقة</button> <button class="btn soft apDecision" data-id="${esc(x.id)}" data-d="reject">رفض</button>`:x.status==='approved'?`<button class="btn soft apQueue" data-id="${esc(x.id)}">إضافة للطابور</button>`:'—'}</td></tr>`).join('')}</tbody></table>`;
      const queue=async b=>{const result=await api(`/api/approvals/${encodeURIComponent(b.dataset.id)}/queue`,{method:'POST',body:'{}'});if(window.showToast)showToast(result.deduplicated?'الإجراء موجود بالفعل في الطابور':'تمت إضافة الإجراء للطابور');};
      box.querySelectorAll('.apDecision').forEach(b=>b.onclick=async()=>{try{await api(`/api/approvals/${encodeURIComponent(b.dataset.id)}/${b.dataset.d}`,{method:'POST',body:'{}'});if(b.dataset.d==='approve')await queue(b);load();}catch(e){window.showToast?showToast(e.message):alert(e.message)}});
      box.querySelectorAll('.apQueue').forEach(b=>b.onclick=async()=>{try{await queue(b);load();}catch(e){window.showToast?showToast(e.message):alert(e.message)}});
    }catch(e){box.className='empty';box.textContent=e.message;}};
