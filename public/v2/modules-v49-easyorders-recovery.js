...
(function(){
  ...
  function summary(result){
    const rows=(result.results||[]).map(x=>{const status=x.error?x.error:(x.rateLimited?'محدود مؤقتًا':'سليم');return `<tr><td>${K.esc(x.connectionId||'—')}</td><td>${K.esc(String(x.recovered||0))}</td><td>${K.esc(String(x.updated||0))}</td><td>${K.esc(String(x.requests||0))}</td><td>${K.esc(status)}</td></tr>`;}).join('');
