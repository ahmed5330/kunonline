/* Kun Online v88.0 — mobile shell hardening + Customer Service resume safety after native phone actions. */
(function(){
  ...
  function rememberResumePoint(){
    if(!mobile()||!activeCustomerService())return;
    const ids=visibleOrderIds();
    resumeSnapshot={ids,focusedOrderId:focusedOrderId()||ids[0]||'',scroll:captureScroll(),at:Date.now()};
  }
  function callLinks(scope=document){
    scope.querySelectorAll?.('a[data-cs-action="call"]').forEach(link=>{
      link.target='_blank';link.rel='noopener';link.dataset.mobileSafeCall='1';
  ...
  async function restoreCustomerService(){
    const snapshot=resumeSnapshot;if(!snapshot||restoring||!mobile()||!activeCustomerService())return false;
    ...
    try{
      ...
      if(missing.length||!root()?.querySelector('.cs-page')){
        ...
        for(let i=0;i<20&&!root()?.querySelector('.cs-page');i++)await new Promise(resolve=>setTimeout(resolve,100));
      }
  ...
  window.KunMobileUXV88={version:'88.0',rememberResumePoint,restoreCustomerService,decorateCallLinks:callLinks,syncNavLock};
