...
(function(){
  ...
  const GROUPS=[
    {id:'sales',label:'المبيعات والعملاء',icon:'sales',accent:'#ff7a59',items:[['orders','الطلبات','sales'],['customer-service','خدمة العملاء','headset'],['customers','إدارة العملاء CRM','users'],['inbox','صندوق الرسائل','message'],['post-shipping','ما بعد الشحن','truck'],['returns-exchanges','المرتجعات والاستبدالات','return']]},
    {id:'stock',label:'المنتجات والتوريد',icon:'box',accent:'#2fcf8f',items:[['products','المنتجات','box'],['inventory','المخزون','warehouse'],['suppliers','الموردون','supplier'],['procurement','المشتريات','cart'],['supplier-finance','حسابات الموردين','finance']]},
    {id:'logistics',label:'الشحن والتحصيل',icon:'truck',accent:'#8b7cf6',items:[['shipping','الشحن','truck'],['cod','تسويات COD','cash']]},
    {id:'growth',label:'التسويق والنمو',icon:'growth',accent:'#f6ad3c',items:[['marketing','أداء الإعلانات','megaphone'],['campaigns','الحملات الإعلانية','target'],['ad-studio','AI Ad Studio','spark'],['analytics','التحليلات والتقارير','chart']]},
    {id:'finance',label:'المالية والربحية',icon:'finance',accent:'#20b8b0',items:[['finance','المالية والربحية','finance'],['accounting','الحسابات والحركات','cash'],['ecommerce-calculator','حاسبة التجارة الإلكترونية','finance'],['profit','تحليل الربحية','growth'],['wallet','المحفظة','wallet']]},
    {id:'ai',label:'الذكاء والأتمتة',icon:'spark',accent:'#4da3ff',items:[['intelligence','مركز الذكاء','spark'],['ai','kun AI','spark'],['automation','الأتمتة','bolt']]},
    {id:'operations',label:'التشغيل والمتاجر',icon:'store',accent:'#38c7d9',items:[['stores','المتاجر والفروع','store'],['store-access','صلاحيات الفروع','shield'],['pos','نقطة البيع POS','pos'],['onboarding','بدء الاستخدام','grid'],['readiness','جاهزية النظام','audit'],['ops','مركز التشغيل','gear']]},
    {id:'system',label:'الإدارة والنظام',icon:'settings',accent:'#a8b4c7',items:[['integrations','مركز التكاملات','plug'],['access','الفريق والصلاحيات','users'],['approvals','مركز الموافقات','shield'],['audit','سجل النشاط','audit'],['control','الحساب والتكاملات','settings'],['settings','الإعدادات','settings'],['admin-clients','Kun Admin','shield']]}
  ];
  ...
  function decorateRoute(button,meta){
    if(!button)return;
    const view=button.dataset.view||'',info=meta||ROUTE_META.get(view)||{label:button.textContent.trim()||view,icon:'grid'};
    if(button.dataset.navDecorated==='v99'&&button.querySelector('.nav-item-icon')&&button.querySelector('.nav-item-label'))return;
    button.innerHTML=routeMarkup(info.label,info.icon);button.dataset.navDecorated='v99';if(info.accent)button.style.setProperty('--nav-accent',info.accent);
  }

  function createGroup(def,buttons){
    const available=def.items.filter(([view])=>buttons.has(view));if(!available.length)return null;
    const group=document.createElement('div');group.className='nav-group';group.dataset.navGroup=def.id;group.style.setProperty('--group-accent',def.accent);
    const toggle=document.createElement('button');toggle.type='button';toggle.className='nav-group-toggle';toggle.setAttribute('aria-expanded','false');toggle.setAttribute('aria-controls',`nav-group-${def.id}`);toggle.innerHTML=`<span class="nav-group-main"><span class="nav-group-icon" aria-hidden="true">${svg(def.icon)}</span><span class="nav-group-label">${def.label}</span></span><span class="nav-group-chevron" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 6 6 6-6 6"/></svg></span>`;
    const items=document.createElement('div');items.className='nav-group-items';items.id=`nav-group-${def.id}`;
    available.forEach(([view,label,icon])=>{const button=buttons.get(view);decorateRoute(button,{label,icon,group:def.id,accent:def.accent});button.classList.add('nav-subitem');button.dataset.navParent=def.id;items.appendChild(button);buttons.delete(view);});
    toggle.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();setOpen(group,!group.classList.contains('is-open'),true);});
    items.addEventListener('click',event=>{const route=event.target.closest('button[data-view]');if(!route)return;setOpen(group,true,true);setTimeout(syncActive,0);},true);
  ...
  function setOpen(group,open,exclusive){
    ...
    group.classList.toggle('is-open',Boolean(open));group.querySelector(':scope > .nav-group-toggle')?.setAttribute('aria-expanded',open?'true':'false');
  ...
  function adoptLateRoutes(nav){
    nav.querySelectorAll('button[data-view]').forEach(button=>{const view=button.dataset.view||'',meta=ROUTE_META.get(view);decorateRoute(button,meta);if(!meta||button.closest('.nav-group'))return;const target=nav.querySelector(`.nav-group[data-nav-group="${CSS.escape(meta.group)}"] .nav-group-items`);if(target){button.classList.add('nav-subitem');button.dataset.navParent=meta.group;target.appendChild(button);}});
    const shortcuts=nav.querySelector('[data-kun-shortcuts-nav]');
    if(shortcuts){if(!shortcuts.querySelector('.nav-item-icon'))shortcuts.innerHTML=routeMarkup('لوحة الاختصارات','grid');shortcuts.dataset.navDecorated='v99';shortcuts.style.setProperty('--nav-accent','#f6c453');shortcuts.classList.add('nav-special-shortcut');}
  }
  ...
  function init(){
    ...
    const dashboard=buttons.get('dashboard');if(dashboard){dashboard.classList.add('nav-standalone','nav-dashboard');decorateRoute(dashboard,{label:'الداشبورد',icon:'dashboard',accent:'#4da3ff'});fragment.appendChild(dashboard);buttons.delete('dashboard');}
