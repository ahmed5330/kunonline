...
(function(){
  ...
  function markRequired(input){if(!input||input.dataset.jt84Required==='1')return;input.dataset.jt84Required='1';const meta=input.closest('label')?.querySelector('.meta');if(meta)meta.textContent='مطلوب لإنشاء الشحنة';input.placeholder='اتركه فارغًا للاحتفاظ بالقيمة الحالية';}
  function enhance(panel){if(!panel||!panel.textContent.includes('J&T Express Egypt'))return;const grid=panel.querySelector('.form-grid');if(!grid)return;
    markRequired(grid.querySelector('[data-secret="customer_code"]'));markRequired(grid.querySelector('[data-secret="customer_password"]'));
