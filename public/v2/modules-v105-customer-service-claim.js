...
(function(){
  ...
  const POLL_MS=5000;
  let timer=null,running=false,lastFingerprint='';
  ...
  function renameShippingSection(){
    const nav=document.querySelector('.nav .post-shipping-nav[data-view="post-shipping"]');if(nav&&nav.textContent.trim()!=='قسم الشحن')nav.textContent='قسم الشحن';
    const page=root()?.querySelector('.ps-page');if(page){const title=page.querySelector('.page-head .title');if(title)title.textContent='قسم الشحن';const sub=page.querySelector('.page-head .sub');if(sub)sub.textContent='جاري الشحن ← تم التوصيل ← جاري التحصيل ← تم التحصيل. متابعة الشحنة والتحصيل بعد خروج الطلب من خدمة العملاء.';}
  }
  ...
  function refreshCounts(){
    ...
    const deferred=root()?.querySelector('.cs-deferred-grid');if(deferred){const count=deferred.querySelectorAll(':scope > .cs-order').length,chip=root()?.querySelector('.cs-deferred-head .cs-count');if(chip)chip.textContent=String(count);ensureEmpty(deferred,'لا توجد طلبات مؤجلة حاليًا');}
  }
  function apply(data){
