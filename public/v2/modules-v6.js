...
function openCodBuilderV6(){
  ...
  document.getElementById('v6SaveCod').onclick=async()=>{try{const id=v4StoreId();const body={clientId:id,orderIds:selected,provider:document.getElementById('v6Provider').value,reference:document.getElementById('v6Reference').value,actualAmount:document.getElementById('v6Actual').value,note:document.getElementById('v6Note').value};await v4ApiPost('/api/cod-reconciliation',body);closeDrawer();toast('تم تسجيل التسوية');codV6()}catch(e){toast(e.message)}};
}
