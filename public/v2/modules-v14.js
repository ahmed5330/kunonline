...
(function(){
  ...
  function decorate(root=document){
    root.querySelectorAll('.title,.k-label,th').forEach(el=>{
      if(el.dataset.helpDecorated)return;const label=el.textContent.trim();const text=HELP[label];if(!text)return;el.dataset.helpDecorated='1';const b=document.createElement('button');b.type='button';b.className='help-dot';b.textContent='?';b.title=text;b.setAttribute('aria-label',`شرح ${label}`);b.onclick=e=>{e.stopPropagation();show(b,label,text)};el.appendChild(b);
    });
  }
