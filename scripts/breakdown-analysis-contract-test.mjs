...
assert(ux.includes("window.KunCampaignHubV66?.state?.sections?.ad?.breakdownData"),'Breakdown analysis must use the currently selected Ads workspace breakdownData');
...
console.log('Selected Meta Breakdown analysis contract passed: current selection is named explicitly, stale analysis is cleared on selection change, delivery/action analysis is separated, and focused values are linked to explainable insights.');
