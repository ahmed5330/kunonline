...
async function api(path,ok=[200]){const r=await fetch(`${base}${path}`,{headers:{...(cookie?{Cookie:cookie}:{})}}),txt=await r.text();let data={};try{data=JSON.parse(txt)}catch{data={raw:txt}}if(!ok.includes(r.status))throw new Error(`GET ${path} expected ${ok.join('/')}, got ${r.status}: ${txt.slice(0,800)}`);return {status:r.status,data};}
...
try{
  await cleanup();
  const candidate=(await d1("SELECT s.client_id,s.id store_id FROM stores s WHERE s.status='active' AND EXISTS(SELECT 1 FROM orders o WHERE o.client_id=s.client_id AND o.store_id=s.id) ORDER BY s.created_at LIMIT 1"))[0]||(await d1("SELECT client_id,id store_id FROM stores WHERE status='active' ORDER BY created_at LIMIT 1"))[0];
  if(!candidate?.client_id||!candidate?.store_id)throw new Error('No active Preview store available for Dashboard live QA');
  ...
  const today=new Date().toISOString().slice(0,10),q=`clientId=${encodeURIComponent(candidate.client_id)}&storeId=${encodeURIComponent(candidate.store_id)}`;
  const daily=(await api(`/api/dashboard?${q}&from=${today}&to=${today}`)).data;assertShape(daily,'daily');if(daily.from!==today||daily.to!==today)throw new Error(`Daily dashboard range mismatch: ${daily.from}..${daily.to}`);
  const all=(await api(`/api/dashboard?${q}&from=beginning&to=${today}`)).data;assertShape(all,'all-time');if(!/^\d{4}-\d{2}-\d{2}$/.test(String(all.from||''))||all.from>today)throw new Error(`All-time dashboard did not resolve a real first date: ${all.from}`);if(Number(all.overview.totalOrders)<Number(daily.overview.totalOrders))throw new Error('All-time order count cannot be lower than today order count');
  await api(`/api/dashboard?${q}&from=${today}&to=2026-01-01`,[400]);
  console.log(`Live Dashboard QA passed: tenant/store scope, today/all-time/custom-range guard, KPI drill-down, finance, ads, return-aware rates, governorates, AI recommendations and trend (${candidate.client_id}/${candidate.store_id}).`);
