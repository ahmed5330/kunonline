...
must(/main\s*=\s*"src\/index-commerce-v10\.js"/.test(wrangler),'Preview must point at current commerce entry v10');
...
for(const expected of ['0000_preview_baseline.sql','0001_preview_schema.sql','0002_profit_cod.sql','0003_approvals_ai_gateway.sql','0004_execution_ops.sql','0005_saas_control_plane.sql','0006_channels_campaigns.sql','0007_pos.sql'])must(migrations.includes(expected),`Missing migration ${expected}`);
...
console.log(`Release readiness checks passed with ${migrations.length} migrations and current v2 assets.`);
