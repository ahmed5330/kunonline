...
const enterpriseResult=await createJtShipment({shipment,secrets,fetcher:async(url,options)=>{
  enterpriseCalls.push({url,options});
  const sent=JSON.parse(new URLSearchParams(options.body).get('bizContent'));
  if(enterpriseCalls.length===1){
...
assert.equal(enterpriseCalls.length,2);assert.equal(enterpriseResult.awb,'JT-ENTERPRISE-1');assert.equal(enterpriseResult.enterpriseRetried,true);
...
const edit=await readFile(new URL('../src/order-edit.js',import.meta.url),'utf8'),worker=await readFile(new URL('../src/index-commerce-v37.js',import.meta.url),'utf8'),v38=await readFile(new URL('../src/index-commerce-v38.js',import.meta.url),'utf8'),ui=await readFile(new URL('../public/v2/modules-v78-jt-shipping-order.js',import.meta.url),'utf8'),setup=await readFile(new URL('../public/v2/modules-v81-jt-live-setup.js',import.meta.url),'utf8'),trackingUi=await readFile(new URL('../public/v2/modules-v82-jt-tracking-cards.js',import.meta.url),'utf8'),recencyUi=await readFile(new URL('../public/v2/modules-v83-order-recency.js',import.meta.url),'utf8'),createSetupUi=await readFile(new URL('../public/v2/modules-v84-jt-create-setup.js',import.meta.url),'utf8'),printingUi=await readFile(new URL('../public/v2/modules-v79-printing.js',import.meta.url),'utf8'),index=await readFile(new URL('../public/v2/index.html',import.meta.url),'utf8');
...
assert.ok(ui.includes('/api/jt/shipments/'),'J&T form must call the live server-side shipment API');assert.ok(!ui.includes('printOrder')&&!ui.includes('window.print'),'Creating a J&T shipment must never issue any print command');assert.ok(!ui.includes('KunCustomerServiceV31?.render?.()'),'J&T shipment must not reload the Customer Service board');assert.ok(ui.includes("moveState?.(orderId,'shipped')"),'Successful J&T creation must move only the order card');
assert.ok(setup.includes('/api/jt/webhook-url'),'J&T setup must expose the logistics callback URL');assert.ok(setup.includes('اختياري — فقط لو J&T طلبته لحسابك'),'J&T Customer Code and Customer Password must be presented as optional Enterprise credentials for Create Order');
assert.ok(!v38.includes('requireCreateBusinessCredentials'),'v38 must not block Create Order before J&T asks for Enterprise credentials');
for(const marker of ['PRINT_ORDER_PATH','/webopenplatformapi/api/order/printOrder',"printSize:'2'",'printCode:1','JT_PRINT_STATE_REQUIRED','JT_LABEL_ALREADY_PRINTED','JT_PRINT_REJECTED','JT_PRINT_URL_MISSING','JT_OFFICIAL_PRINT_REQUIRED','jt_label_printed'])assert.ok(v38.includes(marker),`Official J&T print route missing ${marker}`);
assert.ok(v38.includes("if(!cred.enterpriseReady)")&&v38.includes('JT_BUSINESS_CREDENTIALS_MISSING'),'Business credentials may be required for official print without making them mandatory for Create Order');
for(const marker of ['/api/jt/shipments/${encodeURIComponent(id)}/print','طباعة ونقل إلى تمت الطباعة','في انتظار الطباعة','تمت الطباعة','printOrder'])assert.ok(printingUi.includes(marker),`J&T carrier-authoritative print UI missing ${marker}`);
assert.ok(!printingUi.includes('/printed?')&&!printingUi.includes('data-print79-confirm')&&!printingUi.includes('window.print')&&!printingUi.includes('fallbackPrint'),'Print UI must never locally print or manually mark Printed');
for(const marker of ['sender_name','sender_mobile','sender_prov','sender_city','sender_area','sender_street','Bill Code','markBusinessOptional','Customer Code / Customer Password','عدم وجودهما لا يمنع محاولة إنشاء الشحنة'])assert.ok(createSetupUi.includes(marker),`J&T Create Order setup missing ${marker}`);
...
console.log('J&T live shipping checks passed: standard unprinted create classification and carrier-authoritative printOrder transition are protected.');