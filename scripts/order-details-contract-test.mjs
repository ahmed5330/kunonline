...
for(const marker of ["/api/orders/","/details$","requirePermission(me,'orders','read')",'resolveStoreScope','loadOrderDetails'])must(entry.includes(marker),`order details scoped route missing ${marker}`);
