...
for(const marker of ['AbortController','activeController.abort','aria-busy','campaign71BreakdownRetry','decorateCatalog','مشروط','متوافق تلقائيًا','الطلب اشتغل بنجاح','observerQueued','option.textContent!==desired'])assert(controls.includes(marker),`Reliable Breakdown controls missing marker: ${marker}`);
assert(controls.includes("version:'71.2'")&&controls.includes("dataset.breakdownControls='v71-ready'"),'Breakdown controls must expose v71.2 while keeping a stable readiness token');
assert(reload.includes('modules-v66-campaign-hub.js?v=66.0')&&reload.includes('modules-v67-campaign-comparison-ux.js?v=67.0')&&reload.includes('modules-v68-breakdown-analysis-ux.js?v=68.1')&&reload.includes('modules-v70-breakdown-measurements.js?v=70.1')&&reload.includes('modules-v71-breakdown-controls.js?v=71.2')&&!reload.includes('modules-v65-campaign-hub.js?v=65.0'),'Campaign loader must keep v66 workspaces and load current comparison, analysis, readable measurements and reliable controls layers');
new Function(hub);new Function(comparisonUx);new Function(measurements);new Function(controls);new Function(reload);
console.log('Meta Ads expert analysis contract passed, including independent Campaign/Ad Set/Ad workspaces, per-section dates, sticky comparison metrics, readable creative values, compatible Meta Breakdown requests, per-element measurement cards, idempotent reliable controls, exhaustive filtering, current Meta SDK coverage and tenant/store scoping.');
