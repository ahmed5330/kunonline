...
for(const marker of ['تم التوصيل','POST_SHIPPING_COST_REQUIRED','shipping_cost','shippingCost','actualShippingCost','تكلفة الشحن','atomicDelivery:true'])assert.ok(postShippingV47.includes(marker),`v47 delivered backend missing ${marker}`);
assert.ok(postShippingV47.includes("UPDATE orders SET shipping_cost=?,state='signed'"),'delivery confirmation must persist shipping cost and delivered state atomically');
assert.ok(entry.includes('shippingCost:body.shippingCost??body.shipping_cost'),'v33 post-shipping delivery route must pass the actual shipping cost through the dedicated workflow');
for(const marker of ['تم التوصيل','v47ShippingCost','shippingCost','حفظ السعر وتأكيد التوصيل','/api/post-shipping/orders/','تكلفة الشحن الفعلية',"version:'47.4'"])assert.ok(postShippingUiV47.includes(marker),`v47 delivered UI missing ${marker}`);
assert.ok(!postShippingUiV47.includes('/api/orders/${encodeURIComponent(order.id)}'),'delivery confirmation must not route shipping cost through generic order permissions');
assert.ok(postShippingUiV47.includes('JSON.stringify({clientId,shippingCost})'),'delivery confirmation must submit shipping cost with the dedicated post-shipping request');
...
assert.ok(index.includes('/v2/modules-v47-post-shipping.js?v=47.4'),'v2 shell must load the atomic delivery/shipping-cost post-shipping asset');
...
assert.doesNotThrow(()=>new Function(inventoryUi),'variant inventory price/cost editor must parse');
assert.doesNotThrow(()=>new Function(costUi),'product cost manager browser module must parse');
console.log('Order edit + detailed product catalog + inventory price/cost edit + governed Easy Orders cost review + product cost manager + discounted Easy Orders price sync + atomic delivery shipping-cost gate + post-shipping workflow controls contract passed.');