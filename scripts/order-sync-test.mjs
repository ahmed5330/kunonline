...
for(const marker of ['/api/commerce/order-sync/diagnostics','normalizeEasyOrdersWebhookPayload','webhookLastReceivedAt','webhookLastProbeAt','webhookLastHttpStatus','last_error','method===\'GET\'||method===\'HEAD\'','\\/?$'])assert.ok(worker.includes(marker),`v31 webhook diagnostics missing ${marker}`);
for(const marker of ['provider.modes','orderSyncMode','supported','commerceOrderSync','رابط Webhook','Webhook Secret','saveEasyOrdersWebhookSecret','Public API','Create Webhook','checkEasyOrdersWebhook'])assert.ok(ui.includes(marker),`UI missing ${marker}`);
...
assert.match(preview,/main\s*=\s*"src\/index-commerce-v31\.js"/);
console.log('Commerce order sync contract passed: scoped Easy Orders webhook, wrapper normalization, GET/HEAD readiness, safe diagnostics, tenant/store scoping and secret setup are wired.');
