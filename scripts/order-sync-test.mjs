...
import {reconcileEasyOrdersOrders,easyOrdersReconciliationDefaults} from '../src/easyorders-order-reconciliation.js';
...
for(const marker of ['orders/short/','Api-Key','webhookLastOrderId','easyOrdersRecoveryHighestShortId','easyOrdersRecoveryCursor','MAX_AHEAD_MISSES','rate_limited','handleEasyOrdersWebhook','recoveredOrderIds','maxRequests:DEFAULT_MAX_REQUESTS'])assert.ok(recovery.includes(marker),`recovery missing ${marker}`);
for(const marker of ['/api/commerce/order-sync/reconcile','reconcileEasyOrdersOrders','*/5 * * * *','maxRequests:30','lookback:80','reconcileRecoveredFees'])assert.ok(v34.includes(marker),`v34 recovery wiring missing ${marker}`);
...
console.log('Commerce order sync contract passed: scoped Easy Orders webhook + Short ID gap recovery + five-minute reconciliation + manual repair action are wired.');
