...
const shell=await read('/v2/');markers(shell,'/v2/',['modules-v37-inventory.js?v=37.0']);
...
for(const path of ['/api/inventory/stock-log','/api/commerce/product-import/providers']){const r=await fetch(`${base}${path}`),body=await r.text();if(![401,403].includes(r.status))throw new Error(`${path} must reject anonymous access; got ${r.status}: ${body.slice(0,300)}`);}
console.log('Feature smoke passed: Easy Orders all/selected product import UI + dated inventory/history UI and protected APIs are deployed.');
