...
const shell=await read('/v2/');markers(shell,'/v2/',['modules-v37-inventory.js?v=37.0','modules-v53-stock-clear.js?v=53.0','modules-v54-order-workflow-controls.js?v=54.0']);
...
const orderWorkflow=await read('/v2/modules-v54-order-workflow-controls.js');markers(orderWorkflow,'modules-v54-order-workflow-controls.js',['إدارة الطلب','تحديث الحالة','تعديل الطلب','/api/customer-service/orders/','/api/post-shipping/orders/','KunCustomerServiceV31','KunPostShippingV47']);
...
console.log('Feature smoke passed: Easy Orders product import + dated inventory/history + clear-all stock UI + order-details workflow controls are deployed, and inventory APIs reject anonymous access after Preview propagation stabilization.');
