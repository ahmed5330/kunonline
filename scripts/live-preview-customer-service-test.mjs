...
async function restore(){const errors=[];try{await cleanupClient()}catch(e){errors.push(e.message)}try{if(stateSnapshot)await d1('UPDATE state SET json=?,updated_at=? WHERE id=1',[stateSnapshot.json,stateSnapshot.updated_at])}catch(e){errors.push(e.message)}try{await d1('DELETE FROM login_attempts WHERE email=?',[adminEmail]);await d1('DELETE FROM users WHERE email=?',[adminEmail])}catch(e){errors.push(e.message)}if(errors.length)throw new Error(`Customer Service live cleanup failed: ${errors.join(' | ')}`);}
...
try{
  ...
  const adminCookie=(await login(adminEmail,adminPassword)).cookie;if(!adminCookie)throw new Error('Temporary Customer Service admin cookie missing');
  const onboard=(await api(adminCookie,'/api/admin/clients',{method:'POST',ok:[201],body:{businessName:'CI Customer Service Tenant',ownerName:'CI CS Owner',email:ownerEmail,phone:'01012345678',password:ownerPassword,storeName:'CI CS Store A',plan:'trial',baseOrderFee:0,modules:{dashboard:{enabled:true},stores:{enabled:true},'store-access':{enabled:true},orders:{enabled:true},customers:{enabled:true},team:{enabled:true},settings:{enabled:true}}}})).data;
  clientId=onboard.clientId;storeA=onboard.storeId;if(!clientId||!storeA)throw new Error('Customer Service tenant onboarding failed');
  const ownerCookie=(await login(ownerEmail,ownerPassword)).cookie;if(!ownerCookie)throw new Error('Customer Service owner login failed');
  ...
  const stores=await d1('SELECT id,name FROM stores WHERE client_id=? ORDER BY created_at,id',[clientId]);storeB=stores.find(x=>x.name==='CI CS Store B')?.id;storeC=stores.find(x=>x.name==='CI CS Store C')?.id;if(!storeB||!storeC)throw new Error('Customer Service secondary stores missing');
  ...
  await api(supportCookie,`/api/customer-service/orders/${encodeURIComponent(orderA)}/whatsapp-log?clientId=${encodeURIComponent(clientId)}`,{method:'POST',body:{clientId,storeId:storeA,template:'confirm'}});
  await api(supportCookie,`/api/customer-service/orders/${encodeURIComponent(orderA)}/state?clientId=${encodeURIComponent(clientId)}`,{method:'PATCH',body:{clientId,storeId:storeA,state:'confirmed'}});
  const history=(await api(supportCookie,`/api/customer-service/orders/${encodeURIComponent(orderA)}/history?clientId=${encodeURIComponent(clientId)}&storeId=${encodeURIComponent(storeA)}`)).data.order;
  ...
  const audited=history.history||[];for(const kind of ['contact','internal_note','awb','whatsapp']){const h=audited.find(x=>x.type===kind);if(!h||!String(h.by||'').includes(supportEmail))throw new Error(`Audited Customer Service actor missing for ${kind}: ${JSON.stringify(h)}`);}
  const stateEvent=[...audited].reverse().find(x=>x.state==='confirmed');if(!stateEvent||!String(stateEvent.by||'').includes(supportEmail))throw new Error(`State change actor missing: ${JSON.stringify(stateEvent)}`);
  await api(supportCookie,`/api/customer-service/orders/${encodeURIComponent(orderB)}/state?clientId=${encodeURIComponent(clientId)}`,{method:'PATCH',body:{clientId,storeId:storeB,state:'deferred',deferUntil:tomorrow}});
  board=(await api(supportCookie,`/api/customer-service?clientId=${encodeURIComponent(clientId)}`)).data;const deferred=board.orders.find(x=>String(x.id)===String(orderB));if(deferred?.state!=='deferred'||deferred.deferUntil!==tomorrow)throw new Error(`Deferred order not separated correctly: ${JSON.stringify(deferred)}`);
