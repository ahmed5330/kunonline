...
const expected={worker:'kunonline-preview',entrypoint:'src/index-commerce-v31.js',database:'kunonline-preview',databaseId:'31cd5cdf-fc01-42d7-ba1e-571f3dd58495',binding:'DB'};
...
if(!/name:\s*Live Easy Orders webhook regression[\s\S]*id:\s*easyorders_live[\s\S]*steps\.team_live\.outcome == 'success'[\s\S]*scripts\/live-preview-easyorders-test\.mjs/.test(workflow))throw new Error('Preview safety check failed: live Easy Orders scoped-webhook gate is missing.');
if(!/name:\s*Browser runtime and responsive QA[\s\S]*id:\s*browser[\s\S]*steps\.team_live\.outcome == 'success'[\s\S]*steps\.easyorders_live\.outcome == 'success'[\s\S]*scripts\/browser-preview-qa\.mjs/.test(workflow))throw new Error('Preview safety check failed: browser gate must depend on team and Easy Orders live QA.');
if(!/steps\.smoke\.outcome == 'failure' \|\| steps\.live\.outcome == 'failure' \|\| steps\.team_live\.outcome == 'failure' \|\| steps\.easyorders_live\.outcome == 'failure' \|\| steps\.browser\.outcome == 'failure'/.test(workflow))throw new Error('Preview safety check failed: smoke/live/team/Easy Orders/browser validation failure condition is missing.');
