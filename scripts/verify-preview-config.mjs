...
for(const [label,pattern] of [['Production Wrangler config',/wrangler\.production\.toml/i],['Production D1 command',/\bwrangler\s+d1\b[^\n]*\bkunonline\b(?!-preview)/i],['Production database script',/"db:[^"]*production[^"]*"\s*:/i],['Cloudflare secret mutation',/\bwrangler\s+secret\s+(?:put|bulk|delete)\b/i]])forbidPattern(automation,pattern,`${label} is forbidden.`);
...
for(const [text,message] of [['name: Resolve verified stable Preview source','stable source resolution step is missing.'],['id: stable_source','stable source output step is missing.'],['bootstrap_sha="05787d92d16d5b0da2bdf8dfcdd9a9cf54b875b5"','fully validated Preview CI #841 must remain the bootstrap stable source.'],['refs/tags/preview-stable','preview-stable Git tag lookup is missing.'],['git fetch --no-tags --depth=1 origin "$stable_sha"','stable Git source must be fetched explicitly.'],['echo "source_sha=$stable_sha" >> "$GITHUB_OUTPUT"','stable source SHA must be captured.'],['name: Create fresh verified rollback baseline from stable Git source','fresh same-run baseline creation is missing.'],['id: rollback_base','rollback baseline output step is missing.'],['git worktree add --detach "$stable_dir" "$stable_sha"','stable source must deploy from a detached worktree.'],['CI rollback baseline from stable source $stable_sha','baseline deployment must be identifiable.'],['node scripts/preview-health-check.mjs "$base_url"','fresh baseline must pass health check before candidate deployment.'],['wrangler deployments status --config wrangler.preview.toml --json','Worker traffic ownership must be verified.'],['echo "version_id=$version" >> "$GITHUB_OUTPUT"','fresh baseline/candidate version IDs must be captured.'],['name: Deploy preview Worker','candidate deploy step is missing.'],['Preview candidate $GITHUB_SHA','candidate deploy must identify the tested Git SHA.'],['name: Confirm deployed Worker owns Preview traffic','candidate Worker ownership check is missing.']])requireText(workflow,text,message);
const gates=[['Live authenticated Store B regression','live-preview-functional-test.mjs'],['Live team and branch permission regression','live-preview-team-test.mjs'],['Live Customer Service regression','live-preview-customer-service-test.mjs'],['Live Easy Orders webhook regression','live-preview-easyorders-test.mjs'],['Live Dashboard regression','live-preview-dashboard-test.mjs'],['Browser runtime and responsive QA','browser-preview-qa.mjs']];
...
requireText(workflow,'name: Require every Preview validation gate to succeed','final mandatory QA summary is missing.');requireText(workflow,'id: qa_summary','QA summary output is missing.');
...
requireText(workflow,'if [ "$outcome" != "success" ]','skipped/incomplete QA must count as failure.');requireText(workflow,'echo "all_success=true" >> "$GITHUB_OUTPUT"','QA summary must publish all_success only after every gate passes.');
requireText(workflow,'name: Promote fully validated HEAD to preview-stable','stable promotion step is missing.');requireText(workflow,"if: steps.qa_summary.outputs.all_success == 'true'",'preview-stable must only move after all QA gates succeed.');requireText(workflow,'git tag -f preview-stable "$GITHUB_SHA"','stable tag must point at the fully validated HEAD.');requireText(workflow,'git push origin +refs/tags/preview-stable','stable tag must be persisted to GitHub.');
requireText(workflow,'name: Confirm Worker ownership before rollback','rollback Worker ownership guard is missing.');requireText(workflow,"steps.qa_summary.outcome == 'failure'",'rollback must be driven by final QA failure.');requireText(workflow,'name: Restore same-run verified Preview baseline after failed validation','same-run rollback step is missing.');requireText(workflow,'steps.rollback_base.outputs.version_id','rollback must use the same-run fresh baseline Worker version.');requireText(workflow,'steps.rollback_base.outputs.source_sha','rollback must retain stable Git source provenance.');requireText(workflow,'wrangler versions deploy "$previous@100%" --yes --config wrangler.preview.toml','rollback must restore the same-run verified baseline to 100% traffic.');requireText(workflow,'name: Mark Preview ownership loss','ownership-loss fail-safe is missing.');requireText(workflow,'rollback was intentionally suppressed','ownership loss must suppress unsafe rollback.');
requireText(syncWorkflow,'name: Sync Preview CI/CD','independent sync scheduler CI/CD is missing.');requireText(syncWorkflow,'npm run smoke:sync-preview','sync scheduler live smoke is missing.');
