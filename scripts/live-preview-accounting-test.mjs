...
try{
  ...
  for(const [id,total] of [[order1,1000],[order2,1000],[order3,500]])await d1("INSERT INTO orders (id,client_id,store_id,date,name,phone,total,state,checkpoint,history,created_at) VALUES (?,?,?,?,?,?,?,'pending','جاري التأكيد','[]',?)",[id,clientId,storeId,today,`Customer ${id}`,'01000000000',total,ts]);
  ...
  await patchOrder(order2,'shipped');rows=await feeRows();const b=rows.find(x=>x.orderId===order2);if(!b||b.status!=='active')throw new Error('Second shipped order fee missing');eq(b.ratePct,20,'new order uses new rate');eq(b.amount,200,'second order fee');
  ...
  console.log('Live Accounting QA passed: admin-only per-store management %, charge on shipping, no charge before shipping/cancel-before-ship, reversal on return, historical-rate immutability, dashboard/P&L propagation, accountant entries and store isolation.');
