...
try{
  ...
  const version=(await api('/api/preview/version')).data;if(version.build!=='preview-v33-2026-08-30-order-edit-detailed-products'||version.entrypoint!=='index-commerce-v33.js')throw new Error(`Expected current v33 Preview, got ${JSON.stringify(version)}`);
  ...
  if(!rows.length){console.log('Live Easy Orders webhook QA: legacy route disabled; no connected Easy Orders Preview row exists, so scoped live probe was skipped.');}
  else{
    ...
    console.log(`Live Easy Orders webhook QA passed: connection=${row.id}; externalStoreId=${row.external_store_id||'none'}; secret=${after.secretConfigured?'configured':'missing'}; scopedProbe=200; routeToken=${maskedToken}; lastPOST=${after.webhook?.lastReceivedAt||'none'}.`);
  }
