...
for (const asset of ['/v2/kun-v7.css','/v2/modules-v7.js','/v2/modules-v8.js']) {
  must(index.includes(asset), `Missing ${asset} from v2 index`);
}
for (const theme of ['light','gray','dark']) {
  must(themeJs.includes(`'${theme}'`) || themeJs.includes(`\"${theme}\"`), `Theme ${theme} is missing from switcher`);
}
...
console.log('UI contract checks passed: responsive themes, accessibility, audit and access visibility.');
