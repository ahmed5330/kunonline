...
const clientContextJs = await readFile(new URL('../public/v2/client-context-v24-fast.js', import.meta.url), 'utf8');
...
must(index.includes('/v2/client-context-v24-fast.js'), 'Current client context v24 asset is missing from v2 index');
...
must(clientContextJs.includes('async function resolve(){if(cached)return cached;'), 'Client context v24 must reuse the resolved client within the page instead of repeating network resolution for every API call');
must(clientContextJs.includes("window.KunClientContextV24={version:'24.0'"), 'Client context v24 runtime marker is missing');
