...
for(const marker of ['/api/system/shipping-finance/settings','saveShippingFinancialSettings','snapshotOrderShippingFinance','adjustDashboardForShipping','adjustExpenseDetailsForShipping','enrichShippingFinanceOrders','customerShippingCollected'])assert.ok(backendWrapper.includes(marker),`v38 shipping finance wrapper missing ${marker}`);
for(const marker of ['customer_shipping_charge','carrier_shipping_cost','business_shipping_cost','shipping_finance_mode','shipping_finance_source','productRevenue','customer_full','customer_partial','customer_free','customerObserved','configuredShippingTotal','businessShippingExpense'])assert.ok(shippingBackend.includes(marker),`shipping finance backend missing ${marker}`);
