...
for(const marker of ['inventory_batches','inventory_batch_items','order_stock_allocations','batch_id','batch_name'])assert.ok(batchMigration.includes(marker),`Batch migration missing ${marker}`);
for(const marker of ['listInventoryBatches','createInventoryBatch','assertProductCanDelete','prepareOrderStockTransition','rollbackOrderStockTransition','finalizeOrderStockTransition','remaining_qty','STOCK_BATCH_REQUIRED','PRODUCT_HAS_BATCH_STOCK'])assert.ok(batches.includes(marker),`Stock batch domain missing ${marker}`);
...
console.log('Inventory contract passed: historic dates, named multi-product batches, order allocation, depletion hiding, explicit product/order delete UI and safeguards are wired.');
