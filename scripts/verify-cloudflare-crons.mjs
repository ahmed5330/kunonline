...
const expected=process.argv.slice(3).map(x=>String(x).trim()).filter(Boolean).sort();
if(!accountId||!token)throw new Error('Cloudflare cron verification requires CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN');
if(!worker||!expected.length)throw new Error('Usage: node scripts/verify-cloudflare-crons.mjs <worker> <cron...>');
...
for(let attempt=1;attempt<=6;attempt++){
  ...
  if(JSON.stringify(last)===JSON.stringify(expected)){
    console.log(`Cloudflare cron integrity passed for ${worker}: ${last.join(' | ')}`);
    process.exit(0);
  }
  if(attempt<6)await sleep(5000);
}
throw new Error(`Cloudflare cron integrity failed for ${worker}. Expected [${expected.join(', ')}], got [${last.join(', ')}]. A deploy may have replaced the sync schedules.`);
