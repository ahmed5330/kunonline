...
must(index.includes('/v2/modules-v79-printing.js?v=79.2'),'J&T printing asset must be cache-busted to v79.2');
...
must(printingUi.includes('body>*:not(#print79Sheet){display:none!important}'),'J&T print fallback must remove the application shell from print layout so hidden content cannot create blank pages');
must(printingUi.includes('position:static!important'),'J&T fallback print sheet must remain the only in-flow printable page');
must(printingUi.includes('width:100mm!important;height:150mm!important'),'J&T fallback print document must be constrained to one 100x150mm label page');
for(const marker of ['print79-sidebar-code','FOD','Cdos','REMARKS','FROM:'])must(printingUi.includes(marker),`J&T fallback label missing reference-layout marker ${marker}`);
...
console.log('Integration setup checks passed: governed connections, encrypted credential handoff, idempotent J&T setup, official carrier label printing, faithful one-page fallback and guarded live tracking.');
