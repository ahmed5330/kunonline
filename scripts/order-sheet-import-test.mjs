...
for(const marker of ['order_item_stock_allocations','inventory_batch_items','ORDER BY b.stock_date ASC','date(b.stock_date)<=date(?)','stock_date:order.date','status=\'allocated\'','status=\'returned\''])assert.ok(reconciliation.includes(marker),`Historical stock reconciliation missing ${marker}`);
